				     ...
				     unsigned long size)
{
	...
	if (carveout_heap->pool) {
		...
		pr_debug("carveout alloc:%lx\n", offset);
		if (!offset) {
			pr_err("carveout alloc failed\n");
			  ...
			  phys_addr_t addr, unsigned long size)
{
	if (addr == CARVEOUT_ALLOCATE_FAIL)
		return;
	pr_debug("carveout free size:%lu\n", size);
	down_read(&carveout_heap->mem_sem);
	if (carveout_heap->pool)
		gen_pool_free(carveout_heap->pool, addr, size);
	up_read(&carveout_heap->mem_sem);
	pr_debug("carveout free done\n");
}
						...
						void (*buffer_free)(struct qcom_sg_buffer *))
{
	...
	pr_debug("Carveout allocate:%llx size:%lu\n", paddr, len); 
...
static void carveout_heap_free(struct qcom_sg_buffer *buffer)
{
	...
	pr_debug("carveout heap free len:%lu\n", buffer->len);
				     ...
				     phys_addr_t base, ssize_t size)
{
	...
	pr_debug("Carveout gen pool created %llx\n", base);
	return 0;
}
...
int qcom_carveout_heap_create(struct platform_heap *heap_data)
{
	...
	pr_debug("carveout heap created\n");
	return 0;
