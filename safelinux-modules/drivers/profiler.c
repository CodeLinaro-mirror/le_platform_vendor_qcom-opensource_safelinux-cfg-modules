...
static int bwprofiler_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(profiler->tzmem_pool)) {
		return dev_err_probe(&pdev->dev, PTR_ERR(profiler->tzmem_pool),"profiler: Failed to create qcom_tzmem_pool\n");
	}
	...
	if (rc < 0)
		return dev_err_probe(&pdev->dev, rc, "%s: init chip info failed\n", __func__);
